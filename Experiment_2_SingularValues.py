...
#
# Experiment: Singular Values for Matrix Completion (Chapter 2)
# 
# Bastien Massion
...
for i in range(n_comparisons):
    ...
    clipping = False
    rmse[i][0], wrmse[i][0], singular_values[i][0], acc_mean[i][0], acc_std[i][0], acc_upper_bound, acc_known, out_of_range[i][0], not_adding_to_one[i][0], not_half_on_diagonal[i][0], rank[i][0] = errorMetrics(p_matrix[i][0], p_bar_test, test_selected_omega, test_confrontation_matrix, completion_method, in_range = in_range, add_to_one = add_to_one, diagonal_half = diagonal_half, clipping = clipping, plot_sv = plot_sv)
    
    clipping = True
    rmse[i][1], wrmse[i][1], singular_values[i][1], acc_mean[i][1], acc_std[i][1], acc_upper_bound, acc_known, out_of_range[i][1], not_adding_to_one[i][1], not_half_on_diagonal[i][1], rank[i][1] = errorMetrics(p_matrix[i][1], p_bar_test, test_selected_omega, test_confrontation_matrix, completion_method, in_range = in_range, add_to_one = add_to_one, diagonal_half = diagonal_half, clipping = clipping, plot_sv = plot_sv)
